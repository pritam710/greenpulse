...
export function Operations({ home, Map, staffMode = false }) {
  ...
  return <main className={staffMode ? 'staff' : 'admin'}><header><button onClick={home}>← Home</button><div><h1>{staffMode ? 'Municipal Field Workspace' : 'Admin Operations Centre'}</h1><p>Authenticated, server-recorded workflow</p></div><button onClick={load}>Refresh</button></header>
    ...
    {!staffMode && <><section className="staff-admin"><div><h2>Account administration</h2><p>Create administrator and field-worker accounts. Share temporary passwords privately.</p></div><form onSubmit={createStaff}><label>Full name<input name="name" required maxLength={80}/></label><label>Email<input name="email" type="email" required maxLength={254}/></label><label>Temporary password<input name="password" type="password" required minLength={12} maxLength={128} autoComplete="new-password"/></label><label>Role<select name="role" defaultValue="Driver"><option value="Driver">Field worker</option><option value="Admin">Administrator</option></select></label><button disabled={busy}>Create account</button></form>{user?.is_owner&&<div className="staff-directory"><h3>Owner controls</h3><p>Revoke staff access without deleting audit history.</p>{managedStaff.map(account=><article key={account.id}><span><b>{account.name}</b><small>{account.email} · {account.role === 'Driver' ? 'Field worker' : account.role}</small></span>{account.is_owner?<strong>Owner</strong>:<button type="button" disabled={busy} onClick={()=>revokeStaff(account)}>Revoke access</button>}</article>)}</div>}</section><Map reports={shown}/></>}
